...
export default function Page() {
  ...
  const handleSave = async () => {
    ...
    try {
      if (editItem.id) {
        await api.updateKbItem(clientId, editItem.id, editItem);
      } else {
        await api.createKbItem(clientId, editItem);
      }
      setShowForm(false);
      setEditItem(null);
      fetchItems();
  ...
  const handleDelete = async (id: string) => {
    ...
    try { await api.deleteKbItem(clientId, id); fetchItems(); } catch (err: any) { alert(err.message); }
  };
      ...
      {showForm && (
        <div className="bg-card border border-border rounded-xl p-6 mb-6">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-xs font-medium text-[#888] mb-1">Category</label>
              <input type="text" value={editItem?.category || ''} onChange={e => setEditItem({...editItem!, category: e.target.value})} className="w-full text-sm" />
            </div>
            <div>
              <label className="block text-xs font-medium text-[#888] mb-1">Question</label>
              <input type="text" value={editItem?.question || ''} onChange={e => setEditItem({...editItem!, question: e.target.value})} className="w-full text-sm" />
            </div>
            <div>
              <label className="block text-xs font-medium text-[#888] mb-1">Answer</label>
              <input type="text" value={editItem?.answer || ''} onChange={e => setEditItem({...editItem!, answer: e.target.value})} className="w-full text-sm" />
            </div>
            <div>
